...
class FlowsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create flow" do
    assert_difference('Flow.count') do
      post flows_url, params: { flow: { backgroundImage: @flow.backgroundImage, description: @flow.description, status: @flow.status, teaser: @flow.teaser, title: @flow.title, user_id: @flow.user_id } }
    end
  ...
  test "should update flow" do
    patch flow_url(@flow), params: { flow: { backgroundImage: @flow.backgroundImage, description: @flow.description, status: @flow.status, teaser: @flow.teaser, title: @flow.title, user_id: @flow.user_id } }
